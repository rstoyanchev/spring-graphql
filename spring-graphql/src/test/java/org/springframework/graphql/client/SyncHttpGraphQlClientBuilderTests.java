...
/**
 * Tests for the {@link SyncHttpGraphQlClient} builder performing requests to a
 ...
 * {@link SyncHttpGraphQlClient}.
 *
 * @author Rossen Stoyanchev
 */
public class SyncHttpGraphQlClientBuilderTests {
	...
	void mutateUrlHeaders() {
		...
		// Original
		SyncHttpGraphQlClient.Builder<?> builder = this.setup.initBuilder()
				.url(url)
				.headers(headers -> headers.add("h", "one"));

		SyncHttpGraphQlClient client = builder.build();
	...
	void mutateWebTestClientViaConsumer() {
		...
		// Mutate to add header value
		SyncHttpGraphQlClient.Builder<?> builder2 = client.mutate()
		...
		// Mutate to replace header
		SyncHttpGraphQlClient.Builder<?> builder3 = client.mutate()
	...
	void mutateDocumentSource() {
		...
		// Original
		SyncHttpGraphQlClient.Builder<?> builder = this.setup.initBuilder().documentSource(documentSource);
		SyncHttpGraphQlClient client = builder.build();
	...
	void urlEncoding() {
		...
		SyncHttpGraphQlClient client = this.setup.initBuilder().url("/graphql one").build();
		client.document(DOCUMENT).executeSync();
	...
	private static class ClientBuilderSetup {
		...
		public SyncHttpGraphQlClient.Builder<?> initBuilder() {
			...
			return SyncHttpGraphQlClient.builder(RestClient.builder().requestFactory(requestFactory));
		}
