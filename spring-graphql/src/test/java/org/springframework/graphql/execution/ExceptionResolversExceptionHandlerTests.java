...
public class ExceptionResolversExceptionHandlerTests {
	...
	@BeforeEach
	void setUp() {
		// Ensure snapshot is always present
		DefaultExecutionGraphQlService.captureAndSetContextSnapshot(Context.empty(), this.input);
	}
	...
	void resolveExceptionWithReactorContext() throws Exception {
		...
		DefaultExecutionGraphQlService.captureAndSetContextSnapshot(Context.of("name", "007"), this.input);
